...
function FinancialTab({ financial }: { financial?: SupplierProfile['financial'] }) {
  ...
  return <div className="space-y-4">
    ...
    <section className="bg-[var(--color-surface)] border border-[var(--color-border)] rounded-2xl p-5 shadow-sm"><h3 className="text-sm font-semibold mb-3">偿债与流动性</h3><div className="grid grid-cols-2 sm:grid-cols-4 gap-3"><Metric label="资产负债率" value={percent(financial.debt_ratio)} warn={(financial.debt_ratio ?? 0) > 70} /><Metric label="现金流(亿)" value={financial.cash_flow != null ? financial.cash_flow.toFixed(2) : '-'} warn={(financial.cash_flow ?? 0) < 0} /><Metric label="流动比率" value={financial.current_ratio?.toFixed(2) ?? '-'} /><Metric label="速动比率" value={financial.quick_ratio?.toFixed(2) ?? '-'} /></div></section>
...
function FinancialDetails({ history }: { history: FinancialHistoryItem[] }) {
  ...
  return <section id="financial-details" className="rounded-2xl border border-[var(--color-border)] bg-[var(--color-background)] p-5" aria-label="财务明细">
    ...
    {filteredHistory.length === 0 ? <p className="mt-6 text-center text-sm text-gray-400">当前没有符合筛选条件的报告期数据</p> : <>
      <div className="mt-4 hidden overflow-x-auto md:block"><table className="w-full text-left text-sm"><thead className="border-b border-[var(--color-border)] text-xs text-gray-400"><tr><th className="px-3 py-2 font-medium">报告期</th><th className="px-3 py-2 font-medium">类型</th><th className="px-3 py-2 text-right font-medium">营收</th><th className="px-3 py-2 text-right font-medium">净利润</th><th className="px-3 py-2 text-right font-medium">资产负债率</th><th className="px-3 py-2 text-right font-medium">经营现金流</th></tr></thead><tbody>{filteredHistory.map((item, index) => <tr key={`${item.period}-${index}`} className="border-b border-[var(--color-border)] last:border-0"><td className="px-3 py-3 font-medium">{item.period}</td><td className="px-3 py-3 text-gray-500">{getFinancialPeriodLabel(item.period)}</td><td className="px-3 py-3 text-right">{formatFinancialValue(item.revenue)}</td><td className="px-3 py-3 text-right">{formatFinancialValue(item.net_profit)}</td><td className="px-3 py-3 text-right">{percent(item.debt_ratio)}</td><td className="px-3 py-3 text-right">{formatFinancialValue(item.cash_flow)}</td></tr>)}</tbody></table></div>
      <div className="mt-4 space-y-3 md:hidden">{filteredHistory.map((item, index) => <article key={`${item.period}-${index}`} className="rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] p-3"><div className="flex items-center justify-between gap-3"><span className="font-semibold">{item.period}</span><span className="text-xs text-gray-500">{getFinancialPeriodLabel(item.period)}</span></div><dl className="mt-3 grid grid-cols-2 gap-3 text-sm"><FinancialDetailValue label="营收" value={formatFinancialValue(item.revenue)} /><FinancialDetailValue label="净利润" value={formatFinancialValue(item.net_profit)} /><FinancialDetailValue label="资产负债率" value={percent(item.debt_ratio)} /><FinancialDetailValue label="经营现金流" value={formatFinancialValue(item.cash_flow)} /></dl></article>)}</div>
    </>}
  </section>;
}
