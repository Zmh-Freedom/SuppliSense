...
import { ConclusionSummary, EvidenceTable, ReviewChecklist, TrendSection } from './ChatResultSections';
...
function AgentTrendCharts({ evidence, limitations }: { evidence: AgentEvidenceRecord[]; limitations: string[] }) {
  const businessFacts = evidence.find(record => record.dimension === 'business_risk')?.facts;
          ...
          <div key={i} className={`flex gap-3 ${m.role === 'user' ? 'flex-row-reverse' : ''}`}>
            ...
            <div className={`max-w-[80%] rounded-2xl px-4 py-3 text-sm leading-relaxed shadow-sm ${
              ...
              <div className="prose prose-sm max-w-none prose-p:my-1 prose-headings:my-2 prose-ul:my-1 prose-li:my-0">
                <ReactMarkdown remarkPlugins={[remarkGfm]} components={markdownComponents}>{m.agentAnswer?.summary || m.content}</ReactMarkdown>
              </div>
              {m.role === 'assistant' && m.references && m.references.length > 0 && (
                <details className="group mt-3 border-t border-[var(--color-border)] pt-2">
                  <summary className="flex min-h-10 cursor-pointer list-none items-center justify-between gap-3 py-1 text-left text-xs text-[var(--color-text-secondary)] [&::-webkit-details-marker]:hidden">
                    <span>本轮识别供应商（{m.references.length} 家）</span>
                    <span className="flex items-center gap-1.5"><span>按需展开查看</span><span aria-hidden="true" className="transition-transform group-open:rotate-180">⌄</span></span>
                  </summary>
            ...
            <div className="max-w-[80%] space-y-2">
              ...
              {streamState.answerChunks.length > 0 && (
                <div className="bg-[var(--color-surface)] glass-surface border border-[var(--color-border)] rounded-2xl px-4 py-3 text-sm text-[var(--color-text)] shadow-sm">
                  <div className="prose prose-sm max-w-none prose-p:my-1 prose-headings:my-2 prose-ul:my-1 prose-li:my-0">
                    <ReactMarkdown remarkPlugins={[remarkGfm]} components={markdownComponents}>{streamState.agentAnswer?.summary || streamState.answerChunks.join('')}</ReactMarkdown>
                  </div>
