...
function actionPrompt(target: MonitorTarget): string {
  ...
  if (action === 'verify_identity') return `请核验监控对象“${name}”的主体身份，监控对象ID为 ${id}。只使用有证据支持的数据，并明确本轮未覆盖范围。`;
  if (action === 'assess') return `请对监控对象“${name}”执行首次风险评估，监控对象ID为 ${id}，说明数据覆盖边界。`;
  if (action === 'supplement_data') return `请说明监控对象“${name}”（监控对象ID：${id}）本轮未覆盖的数据域，以及这些限制对风险结论的影响。`;
      ...
      <section className="rounded-2xl border border-[var(--color-border)] bg-[var(--color-surface)] p-5 shadow-sm">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="min-w-0">
            ...
            {target.supplier_id && <Link to={`/suppliers/${encodeURIComponent(target.supplier_id)}`} className="mt-2 inline-flex min-h-[40px] items-center text-xs font-medium text-[var(--color-primary-bg)] hover:underline">查看同一供应商画像 →</Link>}
          </div>
...
function RiskConclusionPanel({ detail, isLoading, error }: { detail?: MonitorRiskDetail; isLoading: boolean; error?: string }) {
  ...
  const judicialStatus = snapshot.risk_detail?.judicial_data_status as Record<string, string> | undefined;
  const judicialLabels: Record<string, string> = { executedPerson: '被执行人', dishonesty: '失信被执行人', courtAnnouncement: '开庭公告', consumptionRestriction: '限制消费' };
  const judicialCounts: Record<string, string> = { executedPerson: 'executed_count', dishonesty: 'dishonesty_count', courtAnnouncement: 'court_announcement_count', consumptionRestriction: 'consumption_restriction_count' };
  ...
  return <section className="rounded-2xl border border-[var(--color-border)] bg-[var(--color-surface)] p-5 shadow-sm">
    ...
    <div className="mt-4 grid gap-2 text-xs sm:grid-cols-2 lg:grid-cols-4" aria-label="风险信息层级"><Layer label="事实" value="本轮快照、报告期与查询状态" /><Layer label="信号" value="风险维度与司法数据状态" /><Layer label="推断" value="当前风险等级与维度判断" /><Layer label="建议" value="下一步采购复核动作" /></div>
...
function Layer({ label, value }: { label: string; value: string }) {
  return <div className="rounded-lg border border-slate-200 bg-slate-50/70 px-3 py-2"><span className="font-medium text-slate-700">{label}</span><span className="ml-1 text-slate-500">{value}</span></div>;
}
...
function IdentityResolutionPanel({
  ...
  return <section className="rounded-2xl border border-indigo-200 bg-indigo-50/60 p-5 shadow-sm">
    ...
    {candidates.length > 0 && <div className="mt-4 space-y-3">{candidates.map(candidate => {
      const canConfirm = candidate.verification_status === 'verified' && Boolean(candidate.company_id);
...
function taskTypeLabel(taskType?: string): string {
  return ({ verify_identity: '主体核验', assess: '首次风险评估', review: '采购风险复核', supplement_data: '数据覆盖核验', continue_monitoring: '继续观察' } as Record<string, string>)[taskType || ''] || taskType || '复核任务';
}
