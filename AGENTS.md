...
- 所有开发、测试、联调、浏览器验收和用户反馈中发现的问题，必须先记录到 `docs/issue-log.md`，再开始修复。
- 问题记录至少包含：发现日期、状态、优先级、现象、影响、根因、修复方案、验证结果和关联提交。
- 修复期间发现的新根因、回归问题或阻塞，必须补充到原问题条目或新增条目；不得只保留在聊天记录、终端日志或个人笔记中。
- 修复完成后必须补充验证结果和提交号；问题记录本身也必须纳入同一提交或关联提交。
- 已关闭问题不得删除；同一问题再次出现时，新增复发记录并关联原问题。
