...
- 状态：待后续任务
- 优先级：P2
- 现象：真实浏览器验收供应商库只读页面时，9 家正式供应商均能从飞书快照加载并显示“来源：飞书”，但分类统一显示“未分类”；同一批供货能力快照在智能寻源结果中已经能正确显示“主营品类”和“供货能力”。
- 影响：供应商库浏览和筛选缺少品类上下文，用户需要进入寻源结果或画像才能了解供应商的供货范围；不影响本次寻源 Agent 返回能力与联系方式。
- 根因：供应商库 `list_suppliers()` 当前只读取供应商主数据快照，没有按稳定 `supplier_id` 关联供货能力快照并聚合分类字段。
- 修复方案：后续在供应商库只读查询层增加当前供货能力快照的按 `supplier_id` 聚合，补充分类、产品和供货区域展示；保持飞书只读边界，不增加写回接口。
- 验证结果：本次先记录问题，不在寻源快照接入任务中扩展修复范围。
- 关联提交：待后续修复。
