...
- [ ] 建立可编程 LLM Double 和脱敏 Provider 录制回放。
- [ ] 实现第 12.2 节全部 P0 场景。
- [ ] 注入超时、429、空结果、冲突、非法输出、数据库提交失败和重启。
- [ ] CI 输出固定质量指标并强制门槛。

验收：P0 指标全部达到第 12.3 节门槛。
...
- [ ] 使用真实开发环境执行完整浏览器场景。
- [ ] 将 `ISS-20260901-020` 至 `ISS-20260901-024` 回填验证和提交。
- [ ] 更新 README、AGENTS、API 与测试说明。
- [ ] 确认仓库中只有本文件声明为活动 Agent 计划。

验收：文档、代码、测试、问题状态和工作台行为一致。
