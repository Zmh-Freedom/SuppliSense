...
验收结果：扩展需求契约和当前轮优先的 Harness 上下文绑定已完成；活动寻源统一调用只读 `discover_supplier_candidates`，固定执行本地历史/Mongo 与飞书正式快照、天眼查、受限联网，返回正式/外部候选、排序分量、联系方式、来源、更新时间和候选级 Evidence/Claim。Task 14 专项 4 项、相关定向 35 项、后端非集成回归 574 项通过；数据库集成测试未在本 Task 执行。关联问题：ISS-20260903-004；关联提交：`a8c3eb3f`。
