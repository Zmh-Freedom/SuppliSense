...
def _create_user_pg(user_data: UserCreate) -> UserInDB:
    from app.domains.auth.user_repo import create_user as pg_create_user, find_by_username, find_by_email
...
def _authenticate_user_pg(username: str, password: str) -> UserInDB | None:
    from app.domains.auth.user_repo import find_by_username
...
def _get_user_by_id_pg(user_id: str) -> UserInDB | None:
    from app.domains.auth.user_repo import find_by_id
...
def _update_user_pg(user_id: str, update_data: UserUpdate) -> UserInDB | None:
    from app.domains.auth.user_repo import update_user as pg_update, find_by_id
...
def _list_users_pg() -> list[UserInDB]:
    from app.domains.auth.user_repo import find_all

    return [UserInDB(**doc) for doc in find_all()]


def _delete_user_pg(user_id: str) -> bool:
    from app.domains.auth.user_repo import delete_user as pg_delete
